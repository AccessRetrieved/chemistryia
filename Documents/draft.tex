...
\indent \indent Increasing temperature increases particles' kinetic energy and the fraction of collisions with energy $\geq E_{A}$, so the rate constant is expected to increase. The temperature dependence of a rate constant can be modelled by the Arrhenius equation:
...
\noindent where $A$ is the frequency factor, $E_{A}$ is the activation energy, $R=8.31\ \text{J}\ \text{K}^{-1}\ \text{mol}^{-1}$\footnote{Value obtained from IB Chemistry Data Booklet, version 1.0 (first assessment 2025).}, and $T$ is the temperature (K).
...
As \ce{O2_{(g)}} is produced in a sealed flask of approximately constant gas volume, the pressure increases (ideal gas approximation):

\begin{equation}
  PV = nRT
\end{equation}

\noindent With \ce{H2O2} concentration and catalyst amount constant, changes in $k_{\text{app}}$ can be compared across temperature levels.
...
\begin{center}
  \textit{How does temperature affect the apparent rate constant for KI-catalyzed \ce{H2O2} decomposition, measured by initial $\Delta P/\Delta t$ in a sealed flask?}
\end{center}
...
\noindent \underline{\textbf{Note:}} As shown in \textit{equation \ref{eq:k_app-equation}}, the apparent rate constant is the same as the initial rate of pressure increase.
...
\noindent \underline{\textbf{Note:}} Trial 5 of $15 ^\circ \text{C}$ was removed from calculations because the source data could not be located on the LabQuest. Run 8 from the LabQuest indicated a target temperature of $20 ^\circ \text{C}$.
...
\noindent Thus, the activation energy for the \ce{KI}-catalyzed decomposition reaction is $60.6 \pm 9.4\ \text{kJ}\ \text{mol}^{-1}$. Note that the uncertainty for activation energy obtained from Excel's LINEST function.

A graph of the mass of \ce{KI} vs. $k_app$ was also plotted to check whether small variations in the mass of \ce{KI} had an effect on the apparent rate constant. However, no clear relationship was observed, which shows that the small variations in the mass of \ce{KI} was not the primary driver of the observed temperature dependence of $k_{app}$:
...
% Overall, increasing temperature increases the apparent rate constant for the decomposition of aqueous hydrogen peroxide measured from the initial pressure-rise rate. The Arrhenius plot is approximately linear ($R^2 = 0.941$), supporting an exponential temperature dependence consistent with collision theory. The activation energy extracted from the Arrhenius slope was $(53.5 \pm 5.5)\ \mathrm{kJ\,mol^{-1}}$, which quantifies the sensitivity of the KI-catalyzed decomposition rate to temperature under the conditions of this investigation.


\section{Conclusion}
\label{sec:conclusion}
This investigation aims to examine the relationship between reaction temperature and the initial rate of the \ce{KI}-catalyze decomposition reaction of hydrogen peroxide, \ce{H2O2}, where the initial rate ($k_{app}$) was determined from the initial slope of pressure ($\text{kPa}$) vs. time ($\text{s}$) in a sealed flask. The experimental data showed that higher temperatures increase both the collision frequency and the fraction of molecules with kinetic energy $\text{KE} \geq \text{E}_{A}$, leading to a larger initial pressure-rise rate as oxygen is produced. The hypothesis in \textit{section \ref{sec:introduction-hypothesis}} predicting an \textbf{exponential relationship} between temperature and the apparent rate constant was supported by the data.

Moreover, given that the apparent rate constant is proportional to the actual rate constant as apparatus setup and initial reactant amounts were held constant, the Arrhenius plot and graph was used, which produced an activation energy of $\text{E}_{A} = 60.6 \pm 9.4\ \text{kJ}\ \text{mol}^{-1}$ for the \ce{KI}-catalyzed pathway. This value is consistent with accepted scientific values online: a lab published by MIT OpenCourseware and the Chemistry LibreTexts reports an uncatalyzed $\text{E}_{A}$ of about $75\ \text{kJ}\ \text{mol}^{-1}$, while iodide catalysis lowers it to about $56$ to $56.5\ \text{kJ}\ \text{mol}^{-1}$ \cite{MIT_OCW_SteamingGun2}\cite{Purdue_Chemed_H2O2}, which is consistent with the experimental data within uncertainty.
...
\indent \indent One major methodological limitaiton lies in the difficulty of maintaining the water bath temperature through the entire $60\ \text{seconds}$ data collection period. While most trials started and ended within $1 ^\circ \text{C}$ of the target temperature, the bath temperature could drift amidst the collection period due to heat loss to the surroundings. Moreover, from personal experience, the temperature probe may not have perfectly represented the temperature of the entire water bath; readings often drifted when blocks of ice drifted nearby. To reduce this limitation, a laboratory thermostatic water bath with internal circulation (i.e. Fisherbrand Isotemp General Purpose Deluxe Water Baths \cite{BathsWaterBaths}, Thermo Scientific Immersion Circulators \cite{ImmersionCirculatorsBaths}, etc.) would provide a more uniform temperature control. However, the high cost of such equipment ($\$1700 - \$4000$) made it impractical for this investigation.
