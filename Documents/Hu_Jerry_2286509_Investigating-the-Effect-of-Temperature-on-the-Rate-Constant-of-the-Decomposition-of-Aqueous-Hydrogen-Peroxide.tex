...
\begin{equation}
  \Delta{\overline{k}_{\mathrm{app}}} = \frac{s_k}{\sqrt{n}}
\end{equation}

\noindent Where $s_k$ is the standard deviation of the 5 $k_{app}$ values, and $n$ is the number of trials (5). This was included because random uncertainties (caused by small variations in, for example, plastic film sealing, mixing time, etc.) can be reduced by taking average, but systematic uncertainties (i.e. constant volunmetric tolerance from using the same pipette for all trials) does not average out.
...
\begin{table}[H]
  ...
  \begin{tabular}{!{\vrule width 1.8pt}
    ...
    >{\centering\arraybackslash}m{1.5cm}!{\vrule width 1.8pt}
      ...
      \textbf{Avg. Initial ($\text{kPa}/\text{s}$)} &
...
\indent \indent Consider the first trial of the first temperature level, $15 ^\circ \text{C}$ as an example (marked as \textcolor{red}{red}, in \textit{table \ref{tab:processed-data}}).
...
This section demonstrates one full calculation of the apparent rate constant $k_{\mathrm{app}}$ from raw pressure-time data, followed by the averaging and random uncertainty calculation for the $15~^{\circ}\mathrm{C}$ level. The same process was applied to all other trials, and the final results are summarized in Table~2.

\subsubsection{Step 1: Convert average temperature to Kelvin}
For Trial 1 at the $15~^{\circ}\mathrm{C}$ level, the recorded average temperature was:
\[
T = 14.75~^{\circ}\mathrm{C}
\]
\[
T = 14.75 + 273.15 = 287.90~\mathrm{K}
\]

\subsubsection{Step 2: Identify the 15 s analysis window}
The analysis window is 15.0~s long, starting at the first continuous pressure rise after the rubber stopper was replaced (defined as a rise sustained for the next 3 adjacent data points). For this trial, the selected interval was:
\[
t_{\text{start}} = 26.6~\mathrm{s}, \quad t_{\text{end}} = 41.6~\mathrm{s}
\]
\[
\Delta t = 41.6 - 26.6 = 15.0~\mathrm{s}
\]

\subsubsection{Step 3: Extract raw pressure data within the window}
Pressure readings $P_i$ (kPa) were taken from the LabQuest export for Run \#4 between $26.6$~s and $41.6$~s.
To keep the sample calculation readable, one data point per second (every 5th data point from the 5~Hz dataset) was used in the table below for demonstration; the full 5~Hz dataset was used for the final slope in Table~2.
...
$t$ / s & $P$ / kPa \\
\hline
26.6 & \underline{\hspace{2cm}} \\
27.6 & \underline{\hspace{2cm}} \\
$\vdots$ & $\vdots$ \\
41.6 & \underline{\hspace{2cm}} \\
\hline
\end{tabular}
\end{table}

\subsubsection{Step 4: Determine $k_{\mathrm{app}}$ from the slope of $P$ vs. $t$}
A linear regression was applied to the points in the selected time window:
\[
P = mt + b
\]
where the slope $m$ is the apparent rate constant:
\[
k_{\mathrm{app}} = m
\]

Using the regression slope from the $P$ vs. $t$ trendline over 26.6--41.6~s:
\[
k_{\mathrm{app},1} = 0.00594~\mathrm{kPa\,s^{-1}}
\]
(This matches the value reported for 15~$^{\circ}$C, Trial 1 in Table~2.)

\subsubsection{Step 5: Calculate the mean $k_{\mathrm{app}}$ at 15$^{\circ}$C}
The five trial values at 15~$^{\circ}$C were:
\[
0.00594,\ 0.00817,\ 0.01237,\ 0.01676,\ 0.00792\ \ (\mathrm{kPa\,s^{-1}})
\]
\[
\overline{k_{\mathrm{app}}} = \frac{0.00594 + 0.00817 + 0.01237 + 0.01676 + 0.00792}{5}
\]
\[
\overline{k_{\mathrm{app}}} = \frac{0.05116}{5} = 0.01023~\mathrm{kPa\,s^{-1}}
\]

\subsubsection{Step 6: Random uncertainty of the mean (standard error)}
The standard deviation of the five trial values at 15~$^{\circ}$C was:
\[
s_k = 0.00434~\mathrm{kPa\,s^{-1}}
\]
The random uncertainty in the mean (standard error) is:
\[
\Delta \overline{k_{\mathrm{app}}} = \frac{s_k}{\sqrt{n}} = \frac{0.00434}{\sqrt{5}} = 0.00194~\mathrm{kPa\,s^{-1}}
\]
Therefore, the rate constant for 15~$^{\circ}$C is:
\[
\overline{k_{\mathrm{app}}} = 0.01023 \pm 0.00194~\mathrm{kPa\,s^{-1}}
\]

\subsubsection{(Optional, if used later) Arrhenius variables}
For Arrhenius analysis using the mean rate constant:
\[
\ln(\overline{k_{\mathrm{app}}}) = \ln(0.01023) = -4.58
\]
Using $T = 287.90~\mathrm{K}$:
\[
\frac{1}{T} = \frac{1}{287.90} = 3.47 \times 10^{-3}~\mathrm{K^{-1}}
\]
